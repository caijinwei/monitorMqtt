...
appModule.controller("infoController", function ($scope, $http, $compile) {
        ...
        T.common.ajax.request('WeconBox', "user/userinfo", new Object(),
            function (data, code, msg) {
                if (code == 200) {
                    ...
                    if (data.type == 1) {
                        $scope.searchbox();
                        $("#ifmain").attr('src', 'webref/html/overview.html');
                    } else if (data.type == 2) {
                        ...
                        $("#ifmain").attr('src', 'web/html/datamonitor.html?device_id=-200&device_name=实时数据');
                    } else {
    ...
    $scope.logout = function () {
        ...
        T.common.ajax.request('WeconBox', "user/signout", new Object(),
            function (data, code, msg) {
                location = "webref/html/user/login.html";
            });
    }

    //左边菜单样式
    $(".leftmenu .J_menuItem").click(function () {
        $(this).parent().parent().parent().addClass("active");
        $(".single").parent().removeClass("active");
    });
     ...
     * 左边菜单样式初始化
     */
    $scope.naninit = function () {
        $("ul.leftmenu").find("li.active").has("ul").children("ul").addClass("collapse in");
        $("ul.leftmenu").find("li").not(".active").has("ul").children("ul").addClass("collapse");

        $("ul.leftmenu>li>a").click(function () {
            if (!$(this).hasClass("single")) {
                $(".single").parent().removeClass("active");
            }
            if ($(this).parent().hasClass("active") && !$(this).hasClass("single")) {
    ...
    $scope.sideclick = function (model) {
        ...
        $a = $('#dir_a_' + model.accountdirId);
        if ($a.children("i").hasClass("fa-folder-o")) {
            $a.children("i").removeClass("fa-folder-o");
            $a.children("i").addClass("fa-folder-open-o");
        } else {
            $a.children("i").removeClass("fa-folder-open-o");
            $a.children("i").addClass("fa-folder-o");
        }

        if ($a.next().hasClass("collapse in")) {
            $a.next().removeClass("in");
        } else {
            $a.next().addClass("collapse in");
        }
    }
